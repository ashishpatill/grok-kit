...
- Skills: `project-bootstrap` (`grok-kit apply` adapts per repo), `plan-execute`, `orchestrate-rlm`, `session-handoff`, `flagship`, `overview`, `visualise`, `code-hygiene`, `memory-sync`, `cost-check`, `refine-harness`, `skill-curator-manual`, `usage-learn`, `verify-aci`, `watch-ci`, `rubric-verify`, `route-task`
...
- Route with `/route-task`. Prove with `/verify-aci`. PR merge-state with `/watch-ci` (status-once). Session start/end: `/flagship` (overview + visualise). Drift: `/code-hygiene` (human decides scrap/keep/fix).
